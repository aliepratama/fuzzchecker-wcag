...
// Prop onCheck dan isLoading sudah dihapus
const InputSection = ({ 
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
      <h2 className="text-2xl font-semibold mb-4 border-b border-slate-200 pb-2">Pengaturan Desain</h2>
      
      {/* ... semua kode input warna dan teks tetap sama ... */}
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center mb-3">
          ...
          { showTextColorPicker && (<div className="absolute z-10"><div className="fixed top-0 right-0 bottom-0 left-0" onClick={() => setShowTextColorPicker(false)}/><ChromePicker color={textColor} onChange={(c) => setTextColor(c.hex)} disableAlpha={true} /></div>)}
        </div>
        <div className="flex justify-between items-center">
          <label className="text-slate-600">Warna Latar</label>
           ...
           { showBgColorPicker && (<div className="absolute z-10"><div className="fixed top-0 right-0 bottom-0 left-0" onClick={() => setShowBgColorPicker(false)}/><ChromePicker color={bgColor} onChange={(c) => setBgColor(c.hex)} disableAlpha={true} /></div>)}
        </div>
      </div>
      <div className="mb-6">
      ...
      {/* Tombol sudah dihapus dari sini */}
    </div>
  );
};
